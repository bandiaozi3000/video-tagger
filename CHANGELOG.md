...
### 新增
- **媒体级一键推荐视频（HTML 预览接入）**：用户手动勾选 Clip 后可从高光工作台打开单媒体 HTML 模板预览；模板展示真实媒体档案、已选片段、跨集章节和回顾墙，并只使用受控片段预览 URL。正式模板录制/导出仍留在 v0.26 后续阶段。
- **高光素材准备与预览基础**：现有工作台支持自动准备可用本地素材、720P 预览、缺源部分版本和高级素材调整；不自动替用户挑片或删片。
- **默认推荐模板**：片头显示媒体封面、标题、年份/季度、外部简介摘要和媒体标签；多片段跨集时自动加入章节卡；片段开始短暂显示集号/标题；片尾显示推荐回顾墙。
- **部分素材预览/导出**：缺源片段明确列出名称、原因和处理入口；可导出当前可用版并在快照/记录中标记部分版本，也可返回补齐素材。
- **快速生成默认值**：默认无 BGM、MP4/1080P；高级的转场、画质、音量和片头片尾开关继续保留并折叠。
- **素材判定统一**：高光准备复用片段素材化的本地资产、BT 下载资产和 Animeko 缓存求值；调整区间时跳过旧 GENERATED_CLIP 产物重新寻找源文件。

### 修复
- **回收站 Bangumi 关联释放**：媒体移入回收站时清除外部作品、集桥接与关联作品缓存，恢复媒体需重新关联；候选禁用仅针对稳定外部 ID 已被其他媒体占用，不再把同名本地媒体误判为已关联。MySQL V33 与 SQLite v15 同步清理历史回收站脏关联。

### 验证
- 推荐/高光/素材化定向测试 49 项通过；Maven 编译、Node 语法和 diff 检查通过。
- 当前 8080 IDEA 实例尚未重启，真实用户素材的预览/部分导出/正式 MP4 黄金路径需重启后补验。
...
### 新增
- **详情页三视图家族统一布局**：媒体/集/片段详情统一「Hero 卡 + 左信息栏 / 右主列表」；媒体详情左栏（档案/素材概览/标签池/收藏夹）+ 右（集列表）；集/片段 hero 封面改横 16:9；列表头 ☰/▦2-4 视图切换（localStorage 记忆）。
- **媒体季（第几季）上移**：media 增 season（默认 1），Episode 删 season——季的唯一权威来源在媒体；季编辑移入媒体编辑弹窗，集编辑只保留集号；Bangumi 同步「创建」媒体时 `TitleParser.parseSeason` 正则从标题/别名识别季（第X季/Season X/第X期，兜底 1），手动编辑与同步更新不覆盖；片段详情展示「第N季 · 第M集」。
- **集/片段档案卡 + 备注内联编辑**：集左栏档案卡（所属媒体可点击跳转/集数/片段数/标记时间）；片段详情左栏档案（所属媒体/所属集可点击/片段时间）+ 素材来源面板接管渠道求值；备注移入 hero 封面右侧、整条可点内联编辑（留空=清空）。
- **本地素材路径展示 + 打开文件夹**：集/片段素材来源面板显示渠道（C1/C2/C3/C4）+ 绝对路径 + 📂 打开文件夹（Electron IPC `vt:show-in-folder`；Web 兜底后端 `POST /api/shell/open-folder`）。
- **批量剪出弹窗升级**：720px 宽、列表/卡片视图切换、封面缩略图、✓已剪出状态徽标。
- **卡片 ⋯ 菜单浮层**：片段卡操作收进 ⋯ 弹出菜单（fixed 定位脱离容器裁剪、视口内自动翻转，规格对齐集行菜单）。

### 双库
- MySQL V31 + SQLite v13：media 加 season 列、episode 删 season 列；sqlite-schema.sql 基线同步 + SqliteSchemaMigrator 推断分支补 v12 判定。

### 修复
- 片段卡 ⋯ 菜单被列表容器裁剪致删除不可见 → 浮层化 fixed 定位；appendClipCard 局部变量遗漏致集片段列表渲染崩溃。
- 集/片段详情按钮 disabled 无置灰观感 → live/disabled 样式扩展进 .hero-ops。
- 档案卡「所属媒体」行值跑中间 → .pv 值组右靠 + cyan 链接胶囊样式。
...
- **媒体详情页 Demo 化重构**：完全按 Demo 布局单容器重写（`#media-detail`），hero 卡片（封面+标题+日文名+按钮行+简介折叠+题材/别名/关联 chip+更多）+ 集列表双列卡 + 右栏（档案信息/素材概览/标签池/收藏夹行式）；按钮改事件委托；外部资料由 renderMediaDetail 直接渲染（绕开 v022 青色主题）。
