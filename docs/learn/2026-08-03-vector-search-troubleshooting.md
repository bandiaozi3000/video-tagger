...
>
> **⚠️ 后续更新（同日）**：第 3.3 节加的 `0.2` 余弦阈值**挡不住乱搜**——真实 embedding 的无关查询 cosine 有模型 baseline（0.24~0.30 全越线）。实测分界约 0.45，已配置化为 `min-cosine-score=0.45`（见 `2026-08-03-milvus-cosine-threshold-calibration.md`）。
