...
class SearchControllerTest {
    ...
    void searchReturnsResults() throws Exception {
        ...
        Mockito.when(searchService.search(eq("战斗"), eq(20), eq(0), eq("mixed"), eq(null), eq(null), eq(null), eq(null), eq(null)))
    ...
    @Test
    void searchPassesSourceFilter() throws Exception {
        SearchResult item = new SearchResult(1L, "某动画 第3集",
                "https://www.bilibili.com/video/BV1", "https://www.bilibili.com/video/BV1?t=754",
                754.5, "高燃战斗", "主角觉醒", 0.032);
        Mockito.when(searchService.search(eq("战斗"), eq(20), eq(0), eq("mixed"), eq(null), eq(null), eq(null), eq(null), eq("OMOFUNA")))
                .thenReturn(new SearchResponse(true, List.of(item)));

        mvc.perform(get("/api/search").param("q", "战斗").param("source", "OMOFUNA"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.results[0].id").value(1));
    }
